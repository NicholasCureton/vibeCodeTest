...
from .core import find_data_dir


# Settings file location
CONFIG_FILE = os.path.join(find_data_dir(__file__), "settings.json")
...
# Params NOT to send to LLM API
LOCAL_ONLY_PARAMS: frozenset = frozenset({"max_agent_iterations"})
            ...
            for key, value in saved.items():
                if key in DEFAULT_SETTINGS:
                    if key in INTEGER_PARAMS:
                        ...
                        # Keep dict/list types as-is (thinking_control, extra_payload, etc.)
                        _settings[key] = value
...
def get_params() -> Dict[str, Union[int, float]]:
    ...
    Excludes local-only params like max_agent_iterations.

    Returns:
        Dict of numeric parameters for LLM API
    """
    _ensure_loaded()
    numeric_keys = (INTEGER_PARAMS | FLOAT_PARAMS) - LOCAL_ONLY_PARAMS
